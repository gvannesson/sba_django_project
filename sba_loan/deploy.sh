...
RESOURCE_GROUP="gvanessonRG"
ACR_NAME="gvannessonregistry"  # Nom de votre Azure Container Registry
REGION="francecentral"  # Région où vous souhaitez déployer
IMAGE_NAME="sba-loan-django-web"  # Nom de l'image Docker
CONTAINER_NAME="gvannesson-sba-loan-django-web"  # Nom du conteneur
...
az container create \
    ...
    --ip-address public \
    --ports 8000 \
    --environment-variables API_USERNAME=$API_USERNAME API_PASSWORD=$API_PASSWORD SECRET_KEY=$SECRET_KEY BASE_URL=$BASE_URL TOKEN=$TOKEN DEBUG=$DEBUG DB_NAME=$DB_NAME DB_USERNAME=$DB_USERNAME DB_PASSWORD=$DB_PASSWORD DB_HOST=$DB_HOST DB_PORT=$DB_PORT DJANGO_ALLOWED_HOSTS=$DJANGO_ALLOWED_HOSTS DJANGO_LOGLEVEL=$DJANGO_LOGLEVEL LOCAL=$LOCAL\
    --registry-login-server $ACR_NAME.azurecr.io \
    --registry-username $REGISTRY_USERNAME \
    --registry-password $REGISTRY_PASSWORD \
    --os-type Linux \
    --dns-name-label gvannessondjango
